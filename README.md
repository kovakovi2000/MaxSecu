...
| `REFUSING: that client folder holds a signed-in account` | You re-ran the client installer in a project folder whose `dist\MaxSecuClient` is a **working app someone has signed in from**. That folder's `keystore\` is the only copy of that account's private key — no server copy, no admin recovery — and laying out a fresh client would delete it, ending the account. **Nothing was changed.** If you meant to *update* an installed client to a newer build, use the upgrade ZIP instead: `powershell -ExecutionPolicy Bypass -File scripts\build-upgrade-zip.ps1`, which replaces only the app and its `ui\` folder and leaves the account, settings and server pin alone. If you meant to make a ZIP for a **new** user, you do not need to re-run anything — mint a registration key and send the `dist\MaxSecuClient-share.zip` that is already there. Only if you genuinely want to throw that client away, re-run with `-ForceReplaceClient` (it still copies the keystore to `dist\_keystore-rescue-<timestamp>\` first). |
