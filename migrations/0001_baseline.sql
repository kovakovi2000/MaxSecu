-- migrations/0001_baseline.sql — the BASELINE.
--
-- This is the schema every already-deployed MaxSecu server is running today. It
-- is a faithful transcription of `docs/schema.sql` (the fresh-install schema),
-- and `crates/compat/tests/schema_equivalence.rs` proves — against a live
-- Postgres — that this file plus every later migration produces EXACTLY the same
-- catalog state as `docs/schema.sql`. A fresh install and an upgraded install
-- must be the same product.
--
-- FROZEN. Never edit an applied migration: `scripts/upgrade-server.sh` records
-- each migration's sha256 in `schema_migrations` and REFUSES to run when a
-- recorded digest no longer matches the file on disk. Editing history would make
-- fresh installs and existing installs permanently divergent. To change the
-- schema, ADD `migrations/NNNN_<slug>.sql`, mirror the change into
-- `docs/schema.sql`, and re-pin `compat/schema.lock`.
--
-- DELIBERATE DIFFERENCES from docs/schema.sql (catalog-identical, none of them
-- changes a single column, constraint, index, trigger or function):
--   1. No `BEGIN;` / `COMMIT;`. The runner wraps each migration in ONE
--      transaction together with its `INSERT INTO schema_migrations`, so applying
--      a migration and recording it are atomic. A `COMMIT;` inside the file would
--      end that transaction early.
--   2. `IF NOT EXISTS` on every CREATE TABLE / CREATE INDEX, and
--      `DROP TRIGGER IF EXISTS` before every CREATE TRIGGER. The very first
--      upgrade of an existing server has an empty `schema_migrations`, so this
--      baseline is (correctly) "pending" and gets applied to a database that
--      already has all of these objects. Idempotency makes that a no-op instead
--      of an error. (`CREATE OR REPLACE FUNCTION` is already idempotent.)
--   3. This header.
--
-- Everything else — table order, column order, types, defaults, CHECKs, PK/FK,
-- comments, index predicates, trigger timing and function bodies — is verbatim
-- from docs/schema.sql.
...
DROP TRIGGER IF EXISTS directory_bindings_immutable ON directory_bindings;
...
CREATE TABLE IF NOT EXISTS auth_nonces (         -- single-use login challenges (§9.2; TTL in parameters.md §2)
...
CREATE TABLE IF NOT EXISTS sessions (             -- channel-bound tokens (§9.2 / api.md §1.5)
...
CREATE TABLE IF NOT EXISTS registration_keys (   -- single-use registration keys for registration-key-only enrollment (T2/T4)
...
CREATE TABLE IF NOT EXISTS first_admin_claim (   -- one-time "first registrant = admin" slot (T4)
...
CREATE TABLE IF NOT EXISTS recovery_account (    -- the ONE escrow identity's PUBLIC keys; registered ONCE (T3)
...
CREATE INDEX IF NOT EXISTS files_listing_idx ON files(file_type, updated_at DESC);  -- D35 listing: sort/filter on type/time only
CREATE INDEX IF NOT EXISTS files_bundle_idx ON files(bundle_id) WHERE bundle_id IS NOT NULL;  -- bundle-member cascade (Task 1.5 delete_file scans WHERE bundle_id = $target)

CREATE TABLE IF NOT EXISTS file_genesis (         -- created once, never modified (§11.7)
...
END $$;
DROP TRIGGER IF EXISTS file_genesis_immutable ON file_genesis;
...
BEGIN
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS file_versions_guard_trg ON file_versions;
...
CREATE TABLE IF NOT EXISTS file_streams (
...
CREATE TABLE IF NOT EXISTS file_key_wraps (
  file_id        BYTEA NOT NULL,
...
CREATE INDEX IF NOT EXISTS wraps_recipient_idx ON file_key_wraps(recipient_id);        -- "files I can read" + sharing-graph projection
...
CREATE UNIQUE INDEX IF NOT EXISTS control_log_revoke_epoch_uq
  ON control_log (COALESCE(scope_file_id, decode('ffffffffffffffffffffffffffffffff','hex')), scope_epoch)
  WHERE kind = 6;
CREATE UNIQUE INDEX IF NOT EXISTS control_log_reinstate_epoch_uq
  ON control_log (COALESCE(scope_file_id, decode('ffffffffffffffffffffffffffffffff','hex')), scope_epoch)
  WHERE kind = 7;
CREATE INDEX IF NOT EXISTS control_log_subject_idx ON control_log (subject_user_id);
CREATE INDEX IF NOT EXISTS control_log_scope_idx   ON control_log (scope_file_id);
...
BEGIN
  ...
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS control_log_append_guard_trg ON control_log;
...
DROP TRIGGER IF EXISTS control_log_immutable ON control_log;
...
CREATE INDEX IF NOT EXISTS auth_events_actor_idx ON auth_events(actor, created_at DESC);
