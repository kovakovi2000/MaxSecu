...
/// One tracked blob: its on-disk filename, byte size, and a monotonic
/// last-used stamp driving LRU eviction.
#[derive(Debug, Clone)]
struct Entry {
    filename: String,
    size_bytes: u64,
...
/// A bounded on-disk LRU of **ciphertext** fragment blobs.
#[derive(Debug)]
...
impl FragmentCache {
    ...
    pub fn put(&mut self, file_id_hex: &str, seq: u32, ciphertext: &[u8]) -> io::Result<()> {
        ...
        let filename = blob_filename(&map_key.0, map_key.1);
        std::fs::write(self.root.join(&filename), ciphertext)?;
        self.tick += 1;
    ...
    pub fn get(&mut self, file_id_hex: &str, seq: u32) -> Option<Vec<u8>> {
        ...
        let map_key = (key, seq);
        let path = self.root.join(&self.index.get(&map_key)?.filename);
    ...
    /// Drop an entry: delete its backing file (best-effort) and adjust totals.
    fn remove_entry(&mut self, key: &(String, u32)) {
        if let Some(e) = self.index.remove(key) {
            let _ = std::fs::remove_file(self.root.join(&e.filename));
