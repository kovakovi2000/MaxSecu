...
| D30 | Media processing locus + decoder safety | **All content understanding — video transcode-to-canonical, thumbnail + short-preview generation, (de)compression — runs **client-side**; the server never decodes. Untrusted (shared) media is decoded in a **sandboxed, no-network, no-key** worker (§8.1)** | The server has no decryption key (§4.3), so every plaintext-derived artifact MUST be produced on the client and stored only as ciphertext. Decoding *shared* media runs complex C codecs (ffmpeg) on **attacker-authored** input — a first-tier RCE surface | A malicious author can still attempt decoder exploits; **contained, not eliminated**, by the sandbox + dimension/size bounds + patched decoders (§8.1, §15.3) |
| D31 | Blob storage tiering | **Small records (manifests, wraps, grants, tombstones, directory) live in Postgres on the server; large ciphertext blobs (content/thumbnail/preview streams) use a **2 TB Dropbox backing tier** with the server's 50–100 GB local disk as an **LRU/LFU cache**. Clients may fetch large blobs **directly from Dropbox** via short-lived, scoped, read-only links** | Bounds server storage; Dropbox is the durable tier. Bytes from cache or Dropbox are **inert ciphertext**, integrity-verified by the signed manifest + per-chunk AEAD tags (§12.3/§12.10), so neither tier is trusted | **Dropbox is a second untrusted observer** of ciphertext sizes/timing/access (§13/§15.2); the server's Dropbox token is a high-value **availability** credential (mass-delete); durability rests on Dropbox (§15.3) |
...
- **Blob storage — two tiers (D31):** large ciphertext blobs (the `content`/`thumbnail`/`preview` streams, chunked) live in a **2 TB Dropbox backing tier**; the server's **50–100 GB local disk is an LRU/LFU cache** (evict oldest-accessed / least-requested). On a cache miss the server fetches from Dropbox (reporting progress to the client) and relays; for large objects the client may instead pull **directly from Dropbox** via a short-lived, scoped, read-only link the server brokers. Both tiers hold **only ciphertext**. Small records stay in the server's Postgres, never in Dropbox.
...
- **Completeness is anchored in the sink, not an online signer (resolves the withholding gap, R16/D22).** The tombstone log is **hash-chained** (each record carries the prior head digest) and its **head is published as part of the sink's digest anchoring** (§16.5). A client requires the served tombstone set to be **contiguous up to the sink-anchored head** and **fails closed on any gap** — so a malicious server can neither *roll back* to a lower epoch nor *withhold* a fresh tombstone past the client's next sink-head refresh. The sink (WORM / independent) is the authority for the head, exactly as it already is for every other "detectable" claim (§11.4). One anchored chain head covers both the `*` and per-file sets — no stapled global epoch, no per-file nonce-challenge.
...
```
manifest = {
  file_id, version, alg, chunk_size,                                         // version increments by exactly 1 from the prior version (§7.5, D23)
...
## 13. Metadata, media streams & compression (resolves M2, per D7; extended by D30/D32/D33)
...
- **Dropbox is a second untrusted observer, and an availability dependency (D31).** Confidentiality holds (inert ciphertext, integrity verified by manifest + AEAD tags), but Dropbox sees ciphertext **sizes/timing/access** like the server (§15.2), and the server's **Dropbox token can mass-delete** the backing store — an availability/DR risk, since the recovery key recovers DEKs, not lost ciphertext. Durability rests on Dropbox; consider an independent ciphertext backup. Scope the token and prefer short-lived per-blob links for direct client fetch.
