...
> **Build status — in progress.** **PQ-hybrid wrap shipped** (P7.1–P7.5: `crypto::hybrid`, `Suite::V2`, ML-KEM-768 keys in bindings/keyblob, uploads emit V2, download/reshare/rotate accept V1+V2). **Recovery-key K-of-N threshold custody shipped** (P7.6 `crypto::shamir` GF(256) primitive; **P7.7** `admin-core::recovery::{split_recovery_key, reconstruct_recovery_key}` wires it into recovery-key custody — the recovery scalar is split `k`-of-`n` and reconstructed only from a threshold of custodian shares at the air-gapped ceremony, proven to open a real recovery wrap; below-threshold reconstruction fails closed; residual = brief air-gapped-RAM reassembly, §16.3). **Real `genesis` anchoring over the HTTP sink shipped** (P7.8: `sink-server` gains a global `PositionLog` + `POST/GET /v1/genesis-anchor` routes that order genesis anchors against control appends in one counter; `server::audit::HttpSinkPublisher::anchor_genesis` is now a real best-effort TLS POST — the R27/D28 key-compromise cutoff basis is real over the sink, proven e2e in `server/tests/sink_publish_e2e.rs`; the client-side `CompromiseCheck` fetch wiring lands with the P7.14 capstone). **Remaining:** the directory key-transparency log (P7.9–P7.12), PQ re-enrollment runbook (P7.13), capstone e2e (P7.14), sign-off (P7.15).
