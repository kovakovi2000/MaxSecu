...
| **Stolen offline recovery device / cold copy (D6)** | Holds **one custodian share** of the recovery private key (no longer a whole cold copy) | With the **K-of-N Shamir custody split shipped** (Phase 7, `admin-core::recovery`, §16.3/§19), a single stolen share — or any `k-1` shares — reveals **nothing**: decrypt-everything now requires a **threshold of custodians** to convene. A legacy whole cold copy, where one still exists, retains the old all-files power, so its risk remains **physical custody**: sealed, dual-custody backup (§16.3). Does **not** by itself grant identity forgery (that needs the directory-signing key D5; D6 is encryption-only). |
...
- **Recovery key (D6):** breakglass, kept **cold** (offline). The recovery private key (an X25519 scalar, §6.3) is now **split K-of-N across custodians with Shamir secret-sharing** (`admin-core::recovery::split_recovery_key` / `reconstruct_recovery_key`, Phase 7 / P7.7, built on the GF(256) `crypto::shamir` primitive from P7.6): any `k` of `n` custodian shares reconstruct the key at the air-gapped ceremony, any `k-1` reveal **nothing**, so **no single safe or person holds the whole key** — a single stolen share or lost share is not total. Each share still lives under the cold-custody controls: **tamper-evident sealing, dual-custody, access logged**. **Documented residual:** the key is *briefly reassembled in air-gapped RAM* at reconstruction (and exposed once on the offline device at split time) — this is the accepted reconstruct-to-use custody posture, not a never-reassemble threshold cryptosystem; all transient copies are zeroized. Wrap/upload is **unchanged** — this is a custody layer over the same recovery key. Plan rotation as a deliberate (expensive) re-wrap project, not an emergency.
...
**Committed to Phase 7 (§17) — promoted out of open-ended "future work" by the second review round (D20). All three shipped (Phase 7):**
