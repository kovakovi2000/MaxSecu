...
- **It is not "no-one can ever decrypt your files."** A standing **recovery recipient** is wrapped into *every* file (§6.3), so whoever physically holds the **offline recovery key (D6) can decrypt 100% of all files, past and present** — a deliberate, disclosed **escrow / breakglass** capability, not an accident. **That "100%" is load-bearing, and since 2026-08-02 it is enforced rather than assumed:** a file owner may not strip the standing recovery recipient. `DELETE /v1/files/{id}/wraps/{recovery_id}` is `403 recovery_protected` for every caller, the owner included (`api.md` §10.2). Until that guard landed the claim was **false** — any authenticated owner could soft-revoke the recovery wrap, and because `add_wrap` refuses to recreate it the blinding was **one-way and permanent**, leaving a file the escrow can never open with nothing recording that it happened. "Zero-knowledge" here means *the server* learns nothing (§15.2), **not** that the operator is cryptographically incapable of recovery. This must be stated plainly in any product copy; users who need operator-incapable secrecy are outside this design's model. The escrow's power is bounded only by custody of the sealed cold key **and its passphrase** (§16.3) — the Shamir/threshold split that was to bound it further shipped in Phase 7 and was then **retired**, so it is unshipped future work again (§19, corrected 2026-08-01).
...
| D20 | Long-term & catastrophic-key risk (review **R6**) | **Elevate PQ-hybrid wrap and recovery-key Shamir split to a committed phase (Phase 7)** | Harvest-now-decrypt-later against X25519 + a single all-files escrow key is the dominant long-horizon risk for a long-lived store; it should not sit in open-ended "future work" | Not in v1; v1 confidentiality is non-PQ and the escrow is a single cold key until Phase 7 ships (§15.2/§15.3). **Outcome recorded 2026-08-01 (the decision above is kept as the historical record):** the PQ-hybrid half shipped and stands (§17); the **recovery-key Shamir-split half shipped and was then RETIRED**, so the "single all-files escrow key" this decision set out to fix is **still the shipped state** (§15.3/§16.3/§19) |
...
| **Stolen offline recovery device / cold copy (D6)** | Holds the **whole** recovery private key as one Argon2id-sealed file (`recovery_key.blob`) | **Corrected 2026-08-01 — the K-of-N Shamir custody split was RETIRED and is not in the code** (`crypto::shamir`, `admin-core::recovery::{split_recovery_key, reconstruct_recovery_key}` do not exist; §16.3). A stolen cold copy is therefore **total again**: the thief who also obtains the **passphrase** can decrypt everything, past and present, with the Argon2id seal (`tools/maxsecu-setup` `setup.rs::seal_recovery_blob`) as the only barrier. Risk is bounded by **physical custody**: sealed, dual-custody backup (§16.3) — there is no threshold and no `k-1` margin. Does **not** by itself grant identity forgery (that needs the directory-signing key D5; D6 is encryption-only). *(Retired claim, kept for provenance — do not act on it.)* ~~With the K-of-N Shamir custody split shipped (Phase 7, `admin-core::recovery`, §16.3/§19), a single stolen share — or any `k-1` shares — reveals nothing: decrypt-everything now requires a threshold of custodians to convene.~~ |
...
| Post-quantum (**Phase 7, IMPLEMENTED** in `crypto::hybrid`) | Hybrid **X25519 + ML-KEM-768** DEK wrap via an X-Wing-style KEM combiner | Both KEM ciphertexts (`eph_x_pub` ‖ ML-KEM `ct`) are bound into the KEK derivation `HKDF-SHA256(ss1 ‖ ss2, info = label ‖ canonical(WrapContext) ‖ eph_x_pub ‖ ct)`, so neither leg can be re-bound and the wrap survives a CRQC unless **both** primitives fall. Each wrap draws a fresh X25519 ephemeral + fresh ML-KEM encaps ⇒ the KEK is single-use, so the AES-256-GCM nonce is the all-zero nonce (safe only under this single-use-KEK invariant). Wire form `eph_x_pub(32) ‖ ct(1088) ‖ aead_ct(48)` = 1168 B; enabled per-record by algorithm agility (V2 suite). |
...
- **One "current" algorithm per purpose.** Exactly one algorithm per purpose is designated current; new uploads always use the current set. **Phase 7 makes `Suite::V2` (the X25519 + ML-KEM-768 hybrid wrap, §5) the current suite once the fleet is PQ-enrolled.** Suite selection is **client-driven and capability-gated** (no server input): a new upload emits `Suite::V2` **iff both the uploader's own identity and the recovery recipient carry an ML-KEM key** (recovery is a mandatory recipient, §6.3, so the PQ guarantee must cover it too); otherwise it falls back to `Suite::V1`, so a **mixed / partially-enrolled fleet still uploads**. The two suites coexist exactly as a migration-in-progress (the fleet-currency reminder above applies until every identity is PQ-enrolled); download/re-share/rotation accept **both** V1 and V2.
...
The recovery key is an asymmetric keypair whose **public** half is a standing recipient: at every upload, the client wraps the DEK to the recovery public key in addition to the human recipients. **"Standing" is enforced at both ends of the wrap's life (2026-08-02).** At upload, a stage without a recovery wrap is refused, and the wrap must set both halves of the recovery pair or neither (`recipient_type == 2 ⇔ recipient_id == RECOVERY_ID` — the biconditional Postgres has enforced since the baseline schema, `api.md` §8.1). Afterwards, **no caller may take it away**: `DELETE /v1/files/{id}/wraps/{recovery_id}` is `403 recovery_protected` for everyone, the file owner included (`api.md` §10.2). That guard is what makes §1.2's *"can decrypt 100% of all files, past and present"* a fact rather than an aspiration — before it, an owner could soft-revoke the recovery wrap, and since `add_wrap` refuses to recreate it the blinding was **one-way and permanent**. Its **private** half lives offline (D6) and is used only to re-derive a DEK during a grant-to-old-file (§12.7) or account recovery (§12.6). The server never holds it, and it is never the live decryptor for served files. Operationally it is a **breakglass admin identity**: its private half is handled exactly like any admin's local key, and every use is authenticated and audited like any privileged action. **Custody: air-gapped is the RECOMMENDED model, no longer the only one (amended 2026-08-01).** The offline device remains the recommended home for the cold key, and the §12.7 grant ceremony still runs there — but the shipped client can also **load the sealed recovery keyblob on a networked machine** and log in over the ordinary channel-bound recovery challenge-response (`client-app/commands/recovery_login.rs`, spec §0 D6 / §6). Loading it that way yields an online session that **browses every file, fetches and decrypts its ciphertext, ends itself, and mints user-role registration keys** (`server/http.rs`'s **five**-handler `RecoveryOkSession` allowlist — `list_files`, `get_file`, `get_chunk`, `chunk_status`, `logout` — plus the pre-existing `AdminSession` path), with no operator involvement and no ceremony. It still cannot upload, delete, revoke, mint a bearer cold-tier link, enumerate a file's other recipients, **or share**, and the session itself carries no key material. **Corrected 2026-08-01 — an earlier revision of this paragraph said the online session "grants any file to any account" over a six-handler allowlist. It does not:** `add_wrap` runs on `AuthedSession` and `403`s the recovery principal.

**Sharing from the recovery identity is a CLOSED DECISION (owner, 2026-08-02).** It is not pending, not blocked on a protocol decision, and not an oversight — the route stays shut, and the reasoning below is *why the decision is final*, not a to-do list. Three reasons, each verified against the code:

1. **A recovery-issued grant is structurally unopenable by its recipient.** The downloader field-binds every ancestor grant before the chain walk and rejects any whose `recipient_type` is not `User` (`client-core/src/download.rs:448-450`); the server serves exactly the recovery wrap's grant as that ancestor.
2. **`granted_by = RECOVERY_ID` resolves to no CLIENT-TRUSTED signing key.** *(Precisely: the recovery account **does** hold an Ed25519 `sig_pub` server-side — `server/src/store.rs:47-51`. Earlier revisions of this paragraph, and of several other documents, said "no signing key exists"; that was **wrong**.)* What is missing is any **trusted path** to it — `GET /v1/recovery/pubkey` serves only `enc_pub`/`mlkem_pub` (`server/src/http.rs:667-686`), the embedded recovery pin omits `sig_pub`, and every client open path passes no-op granter/admin resolvers — so the walk ends at `GrantChainBroken`.
3. **It would DESTROY access that already works.** `add_wrap` is idempotent **by REPLACE** in both stores (`server/src/store.rs:1228`; `server/src/pg.rs:1184-1203` `ON CONFLICT … DO UPDATE`), so a recovery re-share to somebody who already had working access swaps their good grant for one they cannot verify — silently and irreversibly. This is the reason the decision is permanent rather than a missing feature: the failure mode is not "sharing is unbuilt", it is "sharing on this route strands an existing user", which `CLAUDE.md` forbids outright.

Reasons 1 and 2 could in principle be lifted (a published directory binding for `RECOVERY_ID` carrying a real signing key, or `sig_pub` added to the recovery pin — frozen surface #7 — with the chain terminating at the §12.7 admin key), and reason 3 would additionally need a server-side refusal of a REPLACE when `granted_by == RECOVERY_ID`. **The operator declined to take either on.** Restoring a user's access to a file remains the job of the air-gapped §12.7 ceremony, which is what that ceremony is for. **The trade, plainly: the escrow's power is no longer bounded by physical custody of an offline device — it is bounded by custody of the sealed keyblob and its passphrase, plus network reach.** Holding sharing back does not soften that: total remote plaintext of every file is already on the table, and reads are neither audited nor rate-limited. The *only* thing distinguishing the recovery identity from a normal user is that it is a standing recipient on every file; that same exception is why its theft recovers everything (§3.1, §15.3).
...
4. Check `binding.not_before <= now <= binding.not_after` (identity validity — a long lifetime, not a revocation timer).
   - **First-contact key-transparency gate (§7.4, Phase 7).** At *first contact* for a `user_id` (no prior trust-on-last-use record), the Phase-7 client additionally requires that the binding's canonical bytes are **provably included in the directory key-transparency (KT) log** under a checkpoint signed by a **pinned** log key, and that the checkpoint is a non-equivocating advance of the client's persisted KT checkpoint (`client-core::transparency::verify_binding_in_log`). This closes the residual first-contact equivocation window that the TOFU key-change warning (step 3) cannot cover. The gate is opt-in at the API level (`DirectoryVerifier::verify_binding_with_kt`) and fail-closed when configured; a binding that is not provably logged is treated as **absent**.
...
A signed directory stops *forgery* but not *equivocation* (showing different valid bindings to different users) if the **signing key itself** is compromised. The defenses are: air-gapped signing (D5/§12.1), signing-key rotation (§16.4), the **TOFU key-change warning** (§7.2 step 3) — which catches a forged binding on any client that had *previously* wrapped to the victim, turning a silent MITM into a visible re-verify prompt — and, for the first-contact case it cannot cover, the **key-transparency (KT) log**. A stolen D5 can thus MITM only *future* uploads to *not-yet-seen* keys; it cannot touch already-uploaded files or silently re-key a peer a wrapper has seen before.

**Key-transparency log (Phase 7) — the live first-contact equivocation defense.** The directory publishes every binding into an **append-only RFC 6962 log** whose leaves are the canonical `DirBinding` bytes (§7.1). The log periodically signs a **checkpoint** `{tree_size, root}` with a dedicated KT log key (domain-separated under the label `MaxSecu-kt-checkpoint-v1`, distinct from the sink-head control-log checkpoint of §7.6). Clients pin the KT log's public key(s). Two checks, both fail-closed (`client-core::transparency`):

- **Inclusion.** A binding is accepted at first contact only if it is provably the `index`-th leaf under a checkpoint root (RFC 6962 inclusion proof) AND that checkpoint is signed by a pinned KT log key. A directory that serves a *forged* first-contact binding cannot also produce a valid inclusion proof for it without writing it into the public log (where auditors and the victim's own later clients would see it).
- **Consistency / split-view detection (gossip).** Each client persists the latest KT checkpoint it has accepted (trust-on-first-use, mirroring §7.5). A later checkpoint is adopted only if it is a **consistency-proven append-only extension** (RFC 6962 §2.1.2) of the persisted one; a checkpoint that is *not* a genuine extension (a fork served to this client — a **split view**) is rejected as equivocation, and a checkpoint with a lower `tree_size` is rejected as a rollback. Because clients carry their checkpoint forward, an operator who shows a *consistent* log to one client and a *forked* log to another is caught the moment the two checkpoints are cross-checked (gossip), which is the standard transparency-log split-view defense.

The KT inclusion gate is wired into client directory verification as a first-contact step (§7.2 step 4): the Phase-7 client ships it configured; with no KT context supplied the verifier behaves exactly as before (backward-compatible). The in-repo KT **log producer** (the directory-side log that emits checkpoints + proofs) and **enrollment publication** land in P7.11/P7.12; the client verification half (this section) is complete.
...
6. For each recipient `R`: wrap the DEK under the **suite selected by `manifest.alg`** (§5.1), with the same context-bound binding (`info`/KEK over `canonical(wrap_context) = {file_id, version, recipient_id}`, §5) regardless of suite. The **stored wrap is opaque bytes whose wire layout is determined by `alg`**: `Suite::V1` → classical HPKE, `enc(32) ‖ ct`; `Suite::V2` → the X25519 + ML-KEM-768 hybrid wrap, `eph_x_pub(32) ‖ ct_pq(1088) ‖ aead_ct(48)` (1168 bytes). The server stores either verbatim; the downloader picks the parser by the signed `alg` (so the layout cannot be confused or downgraded). The `grant` binds `dek_commit`, **not** the wrap layout, so grants are suite-independent and unchanged.
...
- **Post-quantum: mitigated for V2 uploads; V1 is the residual (Phase 7).** The **hybrid X25519 + ML-KEM-768 wrap shipped** (`Suite::V2`, `crypto::hybrid`, §5/§17/§19): a new upload uses it once both the uploader and the recovery recipient are PQ-enrolled, so a future cryptographically-relevant quantum computer plus *harvested* ciphertext must break **both** legs to recover a V2-wrapped DEK. The residual is **un-migrated V1 files** (and any classical-only identity): X25519-only wraps remain harvest-now-decrypt-later vulnerable, and the standing recovery wrap concentrates that into one key, until the fleet completes PQ re-enrollment (`docs/runbooks/pq-reenrollment.md`) and those files are rotated to V2.
...
- **Recovery device (D6): a single sealed cold copy — theft *plus* passphrase is total.** **Corrected 2026-08-01 — the K-of-N Shamir custody model asserted here was RETIRED and the struck-through text below describes a design that is no longer in the code** (`crypto::shamir` and `admin-core::recovery::{split_recovery_key, reconstruct_recovery_key}` **do not exist**; see §16.3 for the full correction). **What the code actually provides:** one Argon2id-sealed file, `recovery_key.blob`, written once by `tools/maxsecu-setup` (`setup.rs::seal_recovery_blob`) and read by the client at `<app-dir>/recovery/recovery_key_blob` (`client-app/commands/recovery_login.rs::recovery_key_path`). So the pre-Phase-7 residuals stand, unchanged: theft of the cold copy **plus** its passphrase is **total** (decrypt everything, past and present) — the Argon2id seal is the only barrier — and loss of that one copy together with its sealed backup breaks device-loss recovery (§12.6). There is **no threshold**, so no `k-1` safety margin and no survivable share loss. The key is unsealed into RAM at every use, and since the 2026-08-01 amendment (§6.3) that RAM is **no longer air-gapped-only**: the same blob can be loaded on a networked client for an online recovery session. Sealed, dual-custody storage and access logging are the whole of the control; a threshold split is future work (§19).
  - *(Retired description, kept for provenance — do not act on it.)* ~~**Recovery device (D6): no longer a single cold copy (Phase 7), but reassembled in RAM at use.** The recovery private key is now **split K-of-N across custodians with Shamir** (`admin-core::recovery`, §16.3/§19): a single stolen share — or any `k-1` — reveals **nothing**, so theft of one custody item is not total and loss of one share is survivable (any `k` of `n` reconstruct). The residuals: a **legacy whole cold copy**, where one still exists, retains the old all-files power (physical custody is its only bound); and the key is **briefly reassembled in air-gapped RAM** at the reconstruction ceremony (and exposed once at split) — the accepted reconstruct-to-use posture, with all transient copies zeroized. Loss of *more than* `n-k` shares still breaks device-loss recovery (§12.6) absent a sealed backup.~~
...
- **Directory equivocation** by a *compromised signing key* is bounded by air-gapped D5 custody, rotation (§16.4), the **TOFU key-change warning** (§7.2 step 3) — which catches a forged re-key for any pair that has previously wrapped to the victim — and, for the *first-contact* case memory can't range-check, the **directory key-transparency log** (§7.4, **shipped Phase 7**: client inclusion+consistency verification with split-view detection, fail-closed). The residual is now an **ops** one: pinning a long-lived KT log key and gossiping its checkpoints to an independent witness/notary (the in-repo `sink-server::dirlog` producer is the swap-in). Peer pinning was removed (D14) as net complexity for partial coverage.
- **Post-quantum confidentiality: shipped for V2, residual for V1 (Phase 7).** The PQ-hybrid X25519+ML-KEM-768 wrap (`Suite::V2`, §5/§17/§19) is built and is the current suite once an identity + its recovery key are PQ-enrolled — a V2 wrap survives *harvest-now-decrypt-later* unless **both** legs fall. The residual is **un-migrated V1 ciphertext**: X25519-only wraps an attacker copies today are decryptable once a CRQC exists, and the standing recovery wrap concentrates that into one key, until the fleet PQ-enrolls and rotates those files to V2 (`docs/runbooks/pq-reenrollment.md`).
...
- **Recovery key (D6):** breakglass, kept **cold**. **Corrected 2026-08-01 — the K-of-N Shamir custody model described here was RETIRED and the paragraph below describes a design that is no longer in the code.** `crypto::shamir`, `admin-core::recovery::split_recovery_key` and `reconstruct_recovery_key` **no longer exist** (retired with T6 by the trusted-server-recovery redesign, `docs/superpowers/specs/2026-07-03-trusted-server-recovery-registration-design.md` §8; confirmed by the security review's invariant-#7 row). **What the code actually provides today:** the recovery private key exists as **one Argon2id-sealed cold file**, `recovery_key.blob`, written once by `tools/maxsecu-setup` (`setup.rs::seal_recovery_blob`) and opened only with its passphrase; the client reads the same blob at `<app-dir>/recovery/recovery_key_blob` (`client-app/commands/recovery_login.rs::recovery_key_path`). `admin-core::recovery` exports only `build_recovery_grant` and `validate_recovery_wrap`. So custody is back to **a single sealed cold copy plus its passphrase** — tamper-evident sealing, dual-custody and access logging are the whole of the control, there is no threshold, and the §6.3 amendment means that blob is also loadable on a networked machine. A threshold split is once again **future work** (§19), not a shipped property. Wrap/upload is **unchanged** either way — custody is a layer over the same recovery key. Plan rotation as a deliberate (expensive) re-wrap project, not an emergency.
  - *(Retired description, kept for provenance — do not act on it.)* ~~The recovery private key is **split K-of-N across custodians with Shamir secret-sharing** (Phase 7 / P7.7, built on the GF(256) `crypto::shamir` primitive from P7.6): any `k` of `n` custodian shares reconstruct the key at the air-gapped ceremony, any `k-1` reveal **nothing**, so **no single safe or person holds the whole key**. **Documented residual:** the key is *briefly reassembled in air-gapped RAM* at reconstruction (and exposed once on the offline device at split time) — the accepted reconstruct-to-use custody posture, not a never-reassemble threshold cryptosystem; all transient copies are zeroized.~~
...
> **Build status — COMPLETE (all exit gates met).** Control-log records are now **authenticated** client-side, not merely chain-checked: `client-core::revocation::TombstoneSet::verify_authenticated` verifies each record's issuer Ed25519 signature against the resolved directory binding, requires the issuer/co-signer to hold the `admin` **effective** role **as of the chain prefix before that record** (so a de-admin earlier in the chain strips a later record's authority), and enforces dual control on `*`/mass revoke, every reinstatement, and every key-compromise (`sink-interface.md` §5 step 4 — closes the P2.2/P2.3 deferral). The **external sink** is an abstract seam (`client-core::sink`): real `AnchoredHead` + custodian-co-signature `anchor_proof` verification (label `MaxSecu-sink-head-v1`) + `FakeSink`; the real WORM/transparency-log deployment and the transparency-log proof form are **deferred to Phase 6** (the proof allowlist admits them). The download ladder gates on the authenticated set (a **tombstoned author** → `AuthorRevoked`; a revoked recipient → `RecipientRevoked`) and applies the **R27 key-compromise cutoff to genesis by sink position** (`CompromiseCheck`, ignoring the attacker-chosen `created_at`, D28). **Recovery-operator grants** (§12.7, `admin-core::recovery`) are honored on download as an admin-rooted terminal but are **not** carry-forward-eligible (R24). The **R25 subtree walk** (`admin-core::subtree`) is computed from the sink's grant-edge log — a withheld server edge is still tombstoned. Server: the `AuditSink` seam now also `publish_head`s on each control append and `anchor_genesis` at v1 create (global sink positions for R27). Proven e2e over real TLS in `server/tests/sharing_e2e.rs::phase5_revocation_exit_gates_over_real_tls` (served records + sink head → authenticated set; withholding → `Gap`; tombstoned-author → `AuthorRevoked`; dual-controlled reinstatement; R27 genesis-after-compromise → reject). **Deferred to Phase 6:** the real external sink deployment + transparency-log `anchor_proof`; the **offline recovery-wrap validation sweep** (§16.1/D27, R26).
...
> **Build status — COMPLETE (all exit gates met).** The Phase-5 sink/audit seams are now real, transport-exercised components. **New crate `sink-server`** (workspace is now **7 crates**): an append-only `ControlLogStore` (recomputes the head exactly as clients do, rejects rewrite/reorder → 409) + an `Anchorer` emitting BOTH anchor-proof forms, served over `tokio-rustls` via an axum control-log HTTP API (`docs/sink-interface.md` §3). `client-core` gains a real `HttpSinkClient` (behind a default-off `net` feature — the default core stays pure) and `confirm_anchored` (the issuer-side §6 anchoring confirm, fail-closed). The server publishes each control record to the sink (`server::audit::HttpSinkPublisher`, the `AuditSink` seam evolved to carry the record bytes). The **transparency-log `anchor_proof` form** is built (`client-core::sink::AnchorProof::TransparencyInclusion` + RFC-6962 `crypto::merkle`); the custodian co-signature form remains. The **offline recovery-wrap validation sweep** (R26/D27) is `admin-core::{recovery::validate_recovery_wrap, sweep::run_sweep}`. **Monitoring/alerting** is a pure `server::detect` analyzer (§16.5 anomalies) + an `AlertSink` seam. The **sanitized-error pass** (§16.2) added a proof suite (empty-body errors, no existence oracle). **Client-side signed + transparency-logged update verification** is `client-core::update::verify_update` (pinned release key + Merkle inclusion + downgrade/artifact-hash gates, fail-closed). **Reproducible-build** scripts (`scripts/reproducible-build.{sh,ps1}`, `docs/reproducible-builds.md`) — the Linux `media-worker` reproduces byte-identical; **Authenticode signing** + **release/ceremony runbooks** (`scripts/sign-release.ps1`, `docs/runbooks/*`). Proven e2e over real TLS across two independent endpoints in `server/tests/phase6_integrity_ops_e2e.rs` (revoke → publish → fetch+verify → withholding `Gap` → rewrite 409 → R26 sweep catch → sanitized error). No-C posture held (new `axum`/`tokio-rustls`/`hyper` deps pull no second TLS stack; only `aws-lc-rs`). Security review sign-off: `docs/security-review-phase6.md`. **Deferred to ops / Phase 7:** a real third-party WORM/SIEM + cross-publication of the sink head; a live public transparency log / notary (proof shapes + client verification complete; production keys pinned when stood up); ~~genesis anchoring over the real HTTP sink~~ (**done in Phase 7 / P7.8** — `HttpSinkPublisher::anchor_genesis` is now a real best-effort POST to the sink's `POST /v1/genesis-anchor`, ordered against control appends in one global position counter); a real CI runner + Authenticode certificate; the Phase-7 **directory** key-transparency log (§7.4).

**Phase 7 — Long-term hardening (committed, not "someday")**
~~**Recovery-key Shamir/threshold split (§16.3)**~~ (**RETIRED — see the build-status note below**); **PQ-hybrid wrap X25519+ML-KEM-768 via the algorithm-agility path (§5)**; key-transparency log for the directory (§7.4) closing first-contact equivocation. *Exit:* ~~recovery requires a threshold of custodians (no single cold copy is total)~~ (**exit gate retired 2026-08-01**); new uploads use the hybrid wrap; clients detect directory split-views against the log.
> **Build status — COMPLETE (all exit gates met).** **PQ-hybrid wrap shipped** (P7.2–P7.5: `crypto::hybrid` X25519+ML-KEM-768 X-Wing-style combiner — both KEM ciphertexts bound into a single-use KEK; `Suite::V2` + an optional binding `mlkem_pub` field; ML-KEM-768 identity keygen + keyblob **v2** + PQ-signed bindings + directory carry; uploads emit V2 **iff self+recovery are both PQ-enrolled** with mixed-fleet V1 fallback; download/reshare/rotate accept V1+V2 and **fail closed** (`PqKeyMissing`/`ResharePqKeyMissing`) on a missing PQ key — never a silent downgrade). ~~**Recovery-key K-of-N threshold custody shipped** (P7.6 `crypto::shamir` GF(256) primitive; **P7.7** `admin-core::recovery::{split_recovery_key, reconstruct_recovery_key}` — the recovery scalar is split `k`-of-`n` and reconstructed only from a threshold of custodian shares at the air-gapped ceremony, proven to open a real recovery wrap; below-threshold fails closed; residual = brief air-gapped-RAM reassembly, §16.3).~~ **RETIRED — corrected 2026-08-01.** P7.6/P7.7 did ship as described above, and that record is kept for provenance, but the whole threshold-custody stack was **removed** by the later trusted-server-recovery redesign (`docs/superpowers/specs/2026-07-03-trusted-server-recovery-registration-design.md` §8; `crypto/src/shamir.rs` and `admin-core/src/recovery_seal.rs` are deleted, and `admin-core::recovery` now exports only `build_recovery_grant` / `validate_recovery_wrap`). Custody today is **one Argon2id-sealed `recovery_key.blob`** minted by `tools/maxsecu-setup` (§16.3). The other Phase-7 deliverables below are unaffected and still shipped. **Directory key-transparency log shipped** (P7.9 RFC-6962 consistency proofs in `crypto::merkle`; **P7.10** `client-core::transparency` inclusion + consistency verification with **split-view detection** + a first-contact gate `DirectoryVerifier::verify_binding_with_kt`; **P7.11** in-repo producer `sink-server::dirlog` (separate Merkle tree + KT log key, label `MaxSecu-kt-checkpoint-v1`); **P7.12** enrollment publishes bindings to the KT log + an issuer inclusion confirm — all fail-closed, empty-pin never validates). **Real `genesis` anchoring over the HTTP sink shipped** (P7.8: `sink-server` `PositionLog` + admin-gated `POST/GET /v1/genesis-anchor` ordering genesis anchors against control appends in one global counter; `server::audit::HttpSinkPublisher::anchor_genesis` is a real best-effort TLS POST — the R27/D28 cutoff basis is real over the sink). **Client-side R27-comparison over the real sink shipped** (P7.16–P7.18: `sink-server` serves a control record's unified global position at `GET /v1/control-log/position?chain_seq=<n>`; the production `client-core::sink::HttpSinkClient` reads BOTH sides of the cutoff — `fetch_genesis_pos` + `fetch_control_pos` — from the real sink over its pinned TLS channel; `server/tests/phase7_hardening_e2e.rs::r27_cutoff_over_real_sink` proves the full gate end-to-end, sourcing the genesis and `key_compromise` positions from the real sink, no longer `MemoryAuditSink`). **Capstone** (P7.14): the exit gates proven e2e over real TLS in `server/tests/phase7_hardening_e2e.rs` — ~~all three (PQ + threshold recovery + KT split-view)~~ **two, as of the 2026-08-01 correction: PQ + KT split-view.** That file's own header records the threshold gate as "intentionally absent — recovery no longer uses threshold custody" (`phase7_hardening_e2e.rs:3`). No-C posture held (`ml-kem`/`x25519-dalek` are pure-Rust; no new TLS stack — only the pre-existing `aws-lc-rs`); no `unsafe` added. Security review sign-off: `docs/security-review-phase7.md`. **Deferred (ops / future):** the fleet-wide PQ re-enrollment ceremony (runbook `docs/runbooks/pq-reenrollment.md` ready, P7.13); a real third-party KT **witness/notary** + vendor **WORM/SIEM** (the in-repo producers are the swap-in point); a real **CI runner** + **Authenticode certificate**; **ffmpeg video**; **real Dropbox**; **P3.10 zstd**.
...
**Committed to Phase 7 (§17) — promoted out of open-ended "future work" by the second review round (D20). All three shipped (Phase 7); one of them — the Shamir/threshold split — was subsequently **RETIRED** and is open again (corrected 2026-08-01, see below):**
- **Post-quantum hybrid wrap** (X25519 + ML-KEM-768), enabled by algorithm agility (§5) — closes the harvest-now-decrypt-later exposure of v1 (§15.2/§15.3). *Shipped (Phase 7): `crypto::hybrid` X-Wing-style combiner (both KEM ciphertexts bound into a single-use KEK), `Suite::V2` + binding `mlkem_pub` (P7.2–P7.4); uploads emit V2 iff self+recovery are PQ-enrolled, with mixed-fleet V1 fallback, and download/reshare/rotate accept both suites fail-closed (P7.5).*
- **Shamir / threshold split** of the recovery key (removes the D6 single-point-of-theft/loss and the lone-cold-copy total-compromise risk, §15.3/§16.3). **Shipped in Phase 7, then RETIRED — corrected 2026-08-01; this item is OPEN AGAIN.** The trusted-server-recovery redesign removed the whole stack (`docs/superpowers/specs/2026-07-03-trusted-server-recovery-registration-design.md` §8): `crates/crypto/src/shamir.rs` is deleted, `admin-core::recovery` exports only `build_recovery_grant` / `validate_recovery_wrap`, and no Rust source in `crates/` references `split_recovery_key` or `reconstruct_recovery_key`. Custody is back to **one Argon2id-sealed `recovery_key.blob`** (§16.3), so the D6 single-point-of-theft/loss risk this item was meant to remove is live again. *(Retired annotation, kept for provenance — do not act on it.)* ~~Shipped (Phase 7): GF(256) k-of-n secret-sharing primitive (`crypto::shamir`, P7.6) wired into recovery-key custody (`admin-core::recovery::{split_recovery_key, reconstruct_recovery_key}`, P7.7) — the recovery scalar is split k-of-n and reconstructed only from a threshold of custodian shares at the air-gapped ceremony.~~
- **Key transparency log** for the directory (defends against signing-key equivocation for never-met pairs, §7.4) — now the *primary* equivocation defense, since peer pinning was removed (D14). *Shipped (Phase 7): RFC-6962 consistency proofs (`crypto::merkle`, P7.9), client inclusion+consistency verification with split-view detection + a first-contact gate (`client-core::transparency`, P7.10), an in-repo producer (`sink-server::dirlog`, P7.11), and enrollment publishing bindings to the log + an issuer inclusion confirm (P7.12). The real third-party witness/notary is the ops swap-in for the in-repo producer.*
