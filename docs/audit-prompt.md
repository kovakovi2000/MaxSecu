...
Phases **0, 1, 2, 3, 4, 4b, and 5 are claimed COMPLETE** on local `main` (latest on `main`). The workspace is **6 crates**: `encoding`, `crypto`, `client-core`, `admin-core`, `server`, `media-worker`. Your job is to verify that claim end-to-end, not to take it on faith.
...
For the **deferred** items (ffmpeg/dav1d video transcoder, real Dropbox `ColdTier`, zstd encoder, the **real external sink deployment + transparency-log `anchor_proof` form**, the **offline recovery-wrap validation sweep R26/D27**): confirm each is gated honestly (trait returns `CodecUnavailable`/`Unsupported`, seam + fake with the real adapter deferred, or documented as deferred) and is NOT a silent gap masquerading as done.
