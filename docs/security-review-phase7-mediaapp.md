...
| **F1** | `rav1d` `unwrap()` panic on hostile AV1 (`decode.rs:4997`). | Contained DoS | Contained by worker-abort → bounded launcher error; **per-fragment resilience (launcher respawn) now IMPLEMENTED** — see the Addendum below; file an upstream rav1d issue. ASan-clean. |
...
| **Per-fragment crash resilience** | ~~A worker abort (e.g. F1) currently ends the session rather than respawning to skip one fragment.~~ **CLOSED** — launcher-level respawn implemented (`05a53d7`, `5098427`). | Implemented | See the **Addendum** below: a worker-process abort now skips the one culprit fragment and respawns a fresh confined worker for the rest (budget-bounded, confinement preserved, proven e2e). |
...
The residuals — **R1** (AAC audio deferred), **R2** (real ffmpeg ingest deferred behind a default-off feature), **F1/F2** (contained DoS, upstream issues to file), **CF-2** (64 MiB stack, implemented), **large-source delivery**, ~~**per-fragment respawn**~~ (now CLOSED — see the Addendum), and the non-Phase-7 **keystore argon2 host flake** — are deferred honestly with their closure paths. These caveats are the value of this doc: Phase 7 hardens the system's #1 RCE surface as far as the committed pure-Rust path allows, and is explicit about what it does and does not yet prove. **Media-app Phases 1–7 are feature-complete on `media-app`; the `finishing-a-development-branch` merge/PR decision is the natural next step.**

---

## Addendum — Per-fragment crash resilience CLOSED (post-sign-off hardening, `05a53d7` + `5098427`)

The "Per-fragment crash resilience" residual deferred above was subsequently **implemented and dedicated-security-reviewed** (both commits APPROVE, no Critical/High/Medium). A worker-**process** abort mid-window (the kind F1's `rav1d` panic→abort and F2's `symphonia` OOM→Job-kill produce) no longer kills the whole playback window: the launcher **skips the one culprit fragment and respawns a fresh confined worker** for the remainder. This turns F1/F2 from a *window-killing* DoS into a *single-fragment-drop* — a strict availability improvement with **no confinement or zero-knowledge regression**.

- **Mechanism** (`crates/media-launcher/src/lib.rs`): a partial-output driver classifies how a session attempt ended (`Completed` / `WorkerGone` / `Protective`); `resilient_session` resumes after the last `EndOfFragment{seq}` (the culprit = the next, unacked fragment), rebuilds `[Open, <fragments after the culprit>, Close]`, and respawns. Each respawn is a **fresh AppContainer + Job spawn** (no keys, no sockets — confinement byte-identical to the non-resilient path). Cross-respawn output stays globally bounded (`MAX_SESSION_MSGS`/`MAX_SESSION_BYTES` span the whole run, not per-attempt). The decrypted resume plaintext is held in a zeroize-on-drop `ResumeMaterial` guard (wiped on every exit path).
- **DoS bound:** respawns are capped at `MAX_RESPAWNS_PER_WINDOW = 8` **and** the resume cursor strictly advances (skip ≥ 1) each respawn, so even a maximally-hostile clip whose every fragment aborts the worker yields a bounded number of confined spawns then terminates — no amplification.
- **`Protective` defense preserved:** a genuine over-ceiling / undecodable / cap-exceeded signal from a **live** worker on the real frame pipe is still terminal (no respawn) — respawn fires **only** on a true worker-process death (clean frame-pipe EOF + a non-zero/Job-kill exit code).
- **Root-cause win32 fix (`win32.rs`):** the confined `--video-session` worker's `hStdError` is routed to **NULL** (with `STARTF_USESTDHANDLES`) instead of being merged onto the stdout frame pipe — so a dying worker's panic/alloc-failure diagnostics can no longer corrupt the frame stream and masquerade as a `Protective` over-ceiling frame. This neither weakens confinement (a no-stderr device, not console inheritance; the worker holds no secrets to leak) nor changes the `Protective` classification. All confinement suites (`containment_windows`, `containment_video_windows`, `containment_transcode_windows`, `oom_kill_windows`) stay green.
- **Proven e2e:** `crates/media-worker/tests/resilient_oom_windows.rs` drives a real confined `[Open, good0, F2-oom-repro, good1, Close]` window under a 256 MiB Job cap → the middle fragment OOM-kills the confined worker, a fresh confined worker respawns, and **both surrounding fragments decode to `validate_i420`-passing 64×48 frames** (`skipped == [1]`, `respawns == 1`, `terminal == Completed`, no frame from the killed fragment escapes).
- **New Minor (non-blocking):** routing the confined worker's stderr to NULL removes a field-debug channel for *non-fatal* worker misbehaviour. Acceptable (no secrets; merging onto the protocol pipe was the bug). If diagnostics are ever wanted, route stderr to a **separate** dedicated parent-read pipe — never back onto the frame pipe.

This is the only residual closed post-sign-off; the others above remain as stated.
