...
For a proxied fetch that must pull from Dropbox, the server streams the body as it arrives (HTTP/2, chunked) so the client sees throughput; a `GET …/chunks/{index}/status` returns `{ "source":"cache|dropbox-fetching|dropbox-ready", "fetched_bytes":…, "total_bytes":… }` for UI progress (the "fetching from Dropbox" signal — a known popularity/recency side-channel, §15.3, accepted).
