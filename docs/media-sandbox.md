...
## 4. Canonical format — a security choice (✓ RATIFIED, Phase 7 Gate 1)

The uploader **transcodes every file to ONE canonical format before upload** (D30), so a *viewer* only ever decodes that one format and can ship a **single, hardened, minimal decoder** instead of ffmpeg's whole demuxer set. **Pick the canonical format for decoder safety first, compression second.**

> **✓ RATIFIED — Phase 7 Gate 1** (`docs/security-review-phase7-codec-ratification.md`, security-reviewed before adoption). The video canonical set is locked to **AV1 / AAC-LC / CMAF closed-GOP** with a **zero-C, memory-safe view path**: AV1 decode via **`rav1d` 1.1.0** (a memory-safe Rust port of dav1d), and CMAF demux **+ AAC-LC decode** via **`symphonia` 0.6.0** — both pure Rust, `asm` off. Author-side encode is **`rav1e` 0.8.1** (AV1, pure Rust) + ffmpeg's AAC encoder + CMAF mux, confined to the author-ingest worker. **Audio is AAC-LC, not Opus** (the row below originally said Opus) because AAC-LC has a mature pure-Rust decoder and Opus does not — keeping the view path C-free. This is an `alg`-registry addition, **not** a wire-format break. The numeric caps are pinned in `docs/parameters.md` §11.

| Class | Canonical (ratified) | Why (safety-first) |
|---|---|---|
| **Video** | **AV1 in fMP4/CMAF** (key-frame-aligned **closed-GOP fragments**, faststart), decoded with **`rav1d`** (pure-Rust dav1d port) | `dav1d`/`rav1d` is purpose-built for security, **continuously fuzzed**, widely deployed, with a far cleaner memory-safety record than legacy H.264/H.265 C decoders; `rav1d` additionally removes the memory-corruption class **by construction** (pure Rust). AV1 is royalty-free. Container normalized to fragmented, independently-decodable CMAF. |
| **Audio** (in video) | **AAC-LC** in the same MP4/CMAF, decoded with **`symphonia`** (pure Rust) | a single well-reviewed decoder with a **mature pure-Rust** implementation (Opus has none), keeping the view audio path C-free. Author-side AAC encode rides ffmpeg in the confined ingest worker. |
| **Image** | **PNG**, decoded with a **memory-safe Rust decoder** (`png`/`zune-png`); **lossy fallback** transcoded to AVIF (still `dav1d`/`rav1d`) or a constrained JPEG via a memory-safe decoder (`zune-jpeg`) | Prefer **pure-Rust, memory-safe** image decoders so the largest image surface isn't C at all. Keep the *viewer's* image path Rust where possible. |
| **Blog/text** | not media — rendered as **escaped/sanitized** markup, never raw HTML (D24/§8.1) | no decoder; injection-sanitized |

Rationale and tradeoffs:
- **AV1 via `rav1d`** gives the best *decoder-safety-per-format* for video and lets the **view path** be a single, **zero-C / memory-safe** decoder. The heavier **full transcoder** (libav/ffmpeg, many input codecs — the one C carve-out) is needed **only at upload**, on the author's *own* input — also sandboxed, but the author's input is less adversarial than arbitrary shared media.
- **Closed-GOP, independently-decodable fragmented output is mandatory (enables decrypt-while-play + seek).** The canonical container is written **fragmented (CMAF/fMP4, faststart)** with each fragment independently decodable so a viewer can begin playback after the first chunks, **seek** to any fragment, and run a bounded decrypt-while-play window rather than downloading the whole file — the streaming path of `DESIGN.md` §8.1/§12.10. A non-fragmented output (whole `moov` at the end) would force a full download before play and is rejected by the transcode step.
- **Avoid hardware/OS-kernel decoders** on the view path: GPU/driver/Media-Foundation decoders move attacker bytes into kernel/driver space *outside* the AppContainer's containment. Prefer the userspace, sandboxable `rav1d`/Rust decoders even at a CPU cost. (An optional, default-OFF, security-reviewed HW-decode waiver is documented but never shipped enabled.)
- **Prefer memory-safe decoders wherever they exist** (images especially, and now the AV1 view path via `rav1d`): a Rust decoder removes the memory-corruption class for that format outright; reserve C for where no mature memory-safe equivalent exists (the author-side ffmpeg ingest), and keep it patched + fuzzed.
...
> **Ratification record.** Format choice interacts with quality expectations, encode cost, and decoder audit status; AV1/`rav1d` + AAC-LC/`symphonia` + Rust-image is the safety-first set, ratified in Phase 7 Gate 1 with the pinned crate versions, the verified APIs, and the honest residuals (notably **AAC-LC round-trip verified only in Gate 3**, and the **ac-ffmpeg ↔ FFmpeg-8.0 ABI residual** closed in Gate 6) recorded in `docs/security-review-phase7-codec-ratification.md`. The `alg`/format identifier is threaded through the manifest, so a later change is a registry addition, not a wire-format break.
