...
**Tooling:** `maxsecu-admin-core::recovery::build_recovery_grant` (re-wrap a DEK + admin-signed recovery grant, §12.7 steps 3–5).
...
Preconditions: air-gapped recovery machine; D6 present only here; the target file's manifest + the recovery wrap exported by hand.
...
4. **Publish** the new wrap + grant to the app server; **audit** the session (who, file, recipient, timestamp) to the external sink (§16.5).
5. **Re-seal** D6.
