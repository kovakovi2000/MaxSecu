...
1. On the air-gapped recovery machine, generate the recovery recipient's **ML-KEM-768 keypair** alongside its existing X25519 key. The X25519 `enc` key is **reused** as the hybrid classical leg (one X25519 key per binding); only an ML-KEM key is added (`crypto::hybrid::generate_mlkem_keypair`). Custody the new ML-KEM **seed** under the same discipline as the X25519 recovery key (`recovery-key-rotation.md` / §16.3) — the seed is a recovery secret. **Corrected 2026-08-01:** that discipline is ~~K-of-N Shamir~~ **a single Argon2id-sealed cold blob** — the Shamir split was retired and `crypto::shamir` no longer exists (`DESIGN.md` §16.3). In practice the recovery ML-KEM seed already rides inside `recovery_key.blob`: `maxsecu-setup` seals the **whole** recovery `Identity` (X25519 + Ed25519 + the ML-KEM seed) into that one file (`setup.rs::seal_recovery_blob`; blob **v2**, §9.1), so there is no separate seed artefact to custody.
