...
- **PQ-hybrid wrap, key-transparency log** — **SHIPPED in Phase 7** (§17/§19): `crypto::hybrid` X25519+ML-KEM-768 wrap behind the `alg` registry (`Suite::V2`); `client-core::transparency` + `sink-server::dirlog` directory KT log with split-view detection. ~~**Recovery-key Shamir/threshold split** … `crypto::shamir` + `admin-core::recovery` K-of-N recovery-key custody~~ — **RETIRED, corrected 2026-08-01:** it shipped in Phase 7 and was then removed with T6; `crates/crypto/src/shamir.rs` is deleted and `admin-core::recovery` exports only `build_recovery_grant` / `validate_recovery_wrap`. Recovery custody is **one Argon2id-sealed `recovery_key.blob`** minted by `tools/maxsecu-setup` (`DESIGN.md` §16.3); the threshold split is **deferred/future again** (§19). **Still ops/deferred:** the fleet-wide **PQ re-enrollment ceremony** (runbook `docs/runbooks/pq-reenrollment.md` ready) and a real third-party KT **witness/notary** (the in-repo `sink-server::dirlog` producer is the swap-in).
