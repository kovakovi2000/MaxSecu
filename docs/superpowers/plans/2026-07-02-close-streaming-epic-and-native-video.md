...
**Steps (TDD):** adjust/retire the tests that pin the decode path; make `open_video` register-only; delete dead code + crate members; ensure the native range path e2e still passes. Keep `media-transcode-worker` (dev fixtures).
**DECISION (controller, 2026-07-02) — `media-worker` disposition = OPTION (a) from the Task-1 manifest:** `media-worker` is NOT deleted from the workspace. It is **demoted from a shipped product binary to a dev-only verification library**: (1) remove any `client-app` decode spawn path / `worker_path`/`make_decoder` that resolves the `media-worker` binary; (2) stop building/staging `media-worker.exe` in `packaging/package.{ps1,sh}`; (3) KEEP `crates/media-worker` as a workspace member because `media-transcode-worker`'s tests (`ingest_remux.rs`, `containment_transcode_windows.rs`) dev-depend on `maxsecu_media_worker::VideoSession` as a decode-verification oracle for their re-mux output. This preserves decode-verified transcode test coverage and still satisfies the codec-free-client-app invariant (client-app never depended on `media-worker`; it spawned it by path). rav1d/symphonia therefore remain in the workspace (via `media-worker` dev-lib + `symphonia` also stays a real dep of the KEPT `media-transcode-worker`), but MUST NOT be reachable from `client-app`. Removal targets inside `media-launcher` are only the decode-session symbols per the manifest's caution table (`VideoSessionDecoder`/`VideoSubprocessSession`/`AppContainerVideoSession`/`resilient_session*`/`drive_framed_session*`/`spawn_confined_session`); the `framing` module + `spawn_confined`/`spawn_confined_cancellable`/`spawn_confined_exe` + all `TranscodeLauncher`/`FfmpegLauncher` symbols are SHARED with transcode and stay.
**Acceptance:** `cargo build --workspace --tests` clean; native video e2e (`video_e2e.rs::range_streaming_reassembles_plaintext_over_real_tls`) green; `cargo tree -i rav1d` and `cargo tree -i symphonia` show NEITHER is pulled by `client-app` (may still appear under `media-worker`/`media-transcode-worker`); no `decode_and_emit`/`AppContainerVideoSession`/`VideoSessionDecoder` left; `media-transcode-worker` tests still compile+pass (their `media-worker` dev-dep intact). **Security pass:** confirm no decrypt/identity regression in the retained `open_video`/range path (DEK stays in-core; no new oracle).
