...
- [ ] **Step 1:** Add to `VideoJob` a `channel: Option<std::sync::Arc<tokio::sync::Mutex<AuthedChannel>>>` where `pub struct AuthedChannel { pub sender: SendRequest<Full<Bytes>>, pub host: String, pub token: String }` (both `pub` in `jobs.rs` so the e2e can build one). It is `Option` ONLY so the 11 pure `commands::video` unit tests that build a `VideoJob` via `build_job` — none of which serve ranges (they exercise the old decode engine, mostly deleted in Task 7) — can pass `channel: None`; the REAL path (`open_video_inner`) and the e2e always populate `Some`. `serve_range`/`probe_total_len` treat `None` as fail-closed (`player_err`). Populate it in `open_video_inner` from the ONE `reauth` the command already does (move its `sender`/`host`/`token` into the `AuthedChannel` after the open-time header fetch; reauth stays serialized via the UI's `serial()` at open — fine).
- [ ] **Step 2:** Rework `serve_range` to take `jobs` + `file_id_hex` only (drop the `sender/host/token` params): Phase A under the global `VideoJobs` lock → plan + `fetch_indices` + clone the `channel` Arc + read `chunk_size/total_len/index/version`, release the global lock. Phase B → lock the CHANNEL mutex, fetch missing ciphertext over `channel.sender`; **on a send/connection error, return the DISTINCT error code `channel_dead`** (fail-closed; the RECONNECT is the caller's job — see Step 3 — because `reauth` needs app state `serve_range` deliberately does not take, so its signature stays `(jobs, file_id_hex, first, last)` and the Step-4 test can call it directly). Phase C under the global lock → `assemble_range`. This holds neither lock across unrelated work and serializes a session's fetches over its one HTTP/1.1 connection (correct: HTTP/1.1 can't multiplex).
- [ ] **Step 3:** `probe_total_len` uses the session channel too (reworked to `(jobs, file_id_hex, chunk_size)`, locking the job's channel Arc for the last-chunk fetch — no fresh reauth; a `channel_dead` at open just fails the open). `stream_media_inner` no longer calls `reauth`/`server_of` per request and DROPS the bounded busy-retry loop (`076f31a`): it resolves the open session, calls `serve_range`, and **only if `serve_range` returns `channel_dead`** does it `reauth` once, replace the job's `AuthedChannel` contents (lock the channel Arc, `*guard = AuthedChannel { sender, host, token }`), and retry `serve_range` ONCE (a second `channel_dead` ⇒ 500). This is where the persistent connection is (re)built — it is the ONE place with `dir`/`server`/`session`/`connect_lock`.
