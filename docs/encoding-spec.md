...
*(`mlkem_pub` (Phase 7, P7.3) is the optional ML-KEM-768 encapsulation key for Suite-V2 enrollment, carried as a standard `option<…>` (§2). Wire order: it is the **last** field, encoded as the usual 1-byte presence flag — `0x00` absent / `0x01` present, any other byte rejected as `InvalidPresenceByte` exactly like every other optional field — followed, when present, by the fixed **1184**-byte key with no length prefix (the size is structural, like `enc_pub`/`sig_pub`). It is **not** folded into the `fingerprint`, which stays `SHA-256(canonical(fingerprint_input))` over `enc_pub ‖ sig_pub` only; the PQ key is authenticated by the existing D5 Ed25519 signature over `canonical(dirbinding)`, which now covers the trailing field.)*
